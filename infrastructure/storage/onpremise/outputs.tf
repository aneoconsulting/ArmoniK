...
output "mongodb_endpoint_url" {
  value = (contains(module.storage.list_of_storage, "mongodb") ? {
    Status = "READY"
...
output "redis_endpoint_url" {
  value = (contains(module.storage.list_of_storage, "redis") ? {
    Status = "READY"
...
output "activemq_endpoint_url" {
  value = (contains(module.storage.list_of_storage, "amqp") ? {
    Status = "READY"
