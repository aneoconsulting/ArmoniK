...
locals {
  controlplaneServiceName = kubernetes_deployment.control_plane.metadata.0.name
}

resource "kubernetes_job" "stresstest_job" {
  metadata {
    name      = "stresstest-plane"
    namespace = var.namespace
    labels = {
      app     = "armonik"
      service = "stresstest-plane"
    }
  }
  spec {
#   replicas = 1
