locals {
  ...
  credentials = {
    for key, value in {
      Amqp__User = local.queue_storage_adapter_from_secret == "amqp" || local.queue_storage_adapter_from_secret == "rabbitmq"  ? {
        key  = "username"
        name = local.secrets.activemq.name
        } : { key = "", name = "" }
      Amqp__Password = local.queue_storage_adapter_from_secret == "amqp" || local.queue_storage_adapter_from_secret == "rabbitmq" ? {
        key  = "password"
        name = local.secrets.activemq.name
        }  : { key = "", name = "" }
      Amqp__Host = local.queue_storage_adapter_from_secret == "amqp" || local.queue_storage_adapter_from_secret == "rabbitmq" ? {
        key  = "host"
        name = local.secrets.activemq.name
        } : { key = "", name = "" }
      Amqp__Port = local.queue_storage_adapter_from_secret == "amqp" || local.queue_storage_adapter_from_secret == "rabbitmq" ? {
        key  = "port"
        name = local.secrets.activemq.name
        } : { key = "", name = "" }
