locals {
  ...
  needed_storage = {
    object         = var.storage.object == "" ? "" : (contains(local.lower_allowed_storage.object, lower(var.storage.object)) ? lower(var.storage.object) : "mongodb")
    ...
    queue          = var.storage.queue == "" ? "" : (contains(local.lower_allowed_storage.queue, lower(var.storage.queue)) ? lower(var.storage.queue) : "mongodb")
    ...
    shared         = var.storage.shared == "" ? "" : (contains(local.lower_allowed_storage.shared, lower(var.storage.shared)) ? lower(var.storage.shared) : "")
