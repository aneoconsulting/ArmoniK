...
module "seq" {
  ...
  docker_image = {
    image              = local.docker_images["${var.seq.image_name}:${try(coalesce(var.seq.image_tag), "")}"].image
    tag                = local.docker_images["${var.seq.image_name}:${try(coalesce(var.seq.image_tag), "")}"].tag
    image_pull_secrets = var.seq.pull_secrets
  }
  docker_image_cron = {
    image              = local.docker_images["${var.seq.cli_image_name}:${try(coalesce(var.seq.cli_image_tag), "")}"].image
    tag                = local.docker_images["${var.seq.cli_image_name}:${try(coalesce(var.seq.cli_image_tag), "")}"].tag
    image_pull_secrets = var.seq.pull_secrets
  }
...
module "node_exporter" {
  ...
  docker_image = {
    image              = local.docker_images["${var.node_exporter.image_name}:${try(coalesce(var.node_exporter.image_tag), "")}"].image
    tag                = local.docker_images["${var.node_exporter.image_name}:${try(coalesce(var.node_exporter.image_tag), "")}"].tag
...
module "mongodb_exporter" {
  ...
  namespace = local.namespace
  docker_image = {
    image              = local.docker_images["${var.mongodb_metrics_exporter.image_name}:${try(coalesce(var.mongodb_metrics_exporter.image_tag), "")}"].image
...
module "partition_metrics_exporter" {
  ...
  docker_image = {
    image              = local.docker_images["${var.partition_metrics_exporter.image_name}:${try(coalesce(var.partition_metrics_exporter.image_tag), "")}"].image
    tag                = local.docker_images["${var.partition_metrics_exporter.image_name}:${try(coalesce(var.partition_metrics_exporter.image_tag), "")}"].tag
...
module "prometheus" {
  ...
  metrics_exporter_url = "${module.metrics_exporter.host}:${module.metrics_exporter.port}"
  docker_image = {
    image              = local.docker_images["${var.prometheus.image_name}:${try(coalesce(var.prometheus.image_tag), "")}"].image
    tag                = local.docker_images["${var.prometheus.image_name}:${try(coalesce(var.prometheus.image_tag), "")}"].tag
...
module "grafana" {
  ...
  docker_image = {
    image              = local.docker_images["${var.grafana.image_name}:${try(coalesce(var.grafana.image_tag), "")}"].image
    tag                = local.docker_images["${var.grafana.image_name}:${try(coalesce(var.grafana.image_tag), "")}"].tag
