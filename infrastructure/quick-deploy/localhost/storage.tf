...
module "mongodb" {
  ...
  mongodb = {
    ...
    tag                   = try(coalesce(var.mongodb.image_tag), local.default_tags["bitnami/mongodb"])
...
module "mongodb_sharded" {
  ...
  mongodb = {
    image                 = var.mongodb.image_name
    tag                   = try(coalesce(var.mongodb.image_tag), local.default_tags["bitnami/mongodb-sharded"])
    node_selector         = var.mongodb.node_selector
    image_pull_secrets    = var.mongodb.image_pull_secrets
