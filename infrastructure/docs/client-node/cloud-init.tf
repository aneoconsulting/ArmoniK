...
data "template_cloudinit_config" "client_cloud_init" {
  ...
  part {
    filename   = "0007-python3.yml"
    content    = templatefile("cloud-init-templates/0007-python3.yaml", {})
    merge_type = var.extra_userdata_merge
  }

  part {
    filename   = "0008-ssh.yml"
    content    = templatefile("cloud-init-templates/0008-ssh.yaml", {})
    merge_type = var.extra_userdata_merge
  }
  part {
    filename   = "0009-dotnet.yml"
    content    = templatefile("cloud-init-templates/0009-dotnet.yaml", {})
    merge_type = var.extra_userdata_merge
  }
}