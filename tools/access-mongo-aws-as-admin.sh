echo "***** this script allows you to connect to mongo directly from inside the cluster => useful for AWS installation *****"
echo "Firstly you have to connect to db : use database" && \
echo "Then you can try requests ex : " && \
echo "Display all TaskData : db.TaskData.find().pretty() " && \
echo "db.TaskData.find({ SessionId: { $eq : '7eafe4e3-0aa2-46ef-8ce6-bf9e365c5449' }, ExpectedOutputIds: { $eq : 'a600dca5-b672-4177-9b4a-880dbcefee4e'}}).pretty()" && \
