locals {
  ...
  rsmq_pod_ip = kubernetes_service.rsmq.status.0.load_balancer.0.ingress.0.ip == "" ? lookup(tomap(data.external.external_ip.result), "external_ip", "localhost") : kubernetes_service.rsmq.status.0.load_balancer.0.ingress.0.ip
}
