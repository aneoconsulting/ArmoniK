...
Each key in `compute_plane` defines one partition. See the [partitioning guide](./2.how-to-configure-partitions.md) for a full discussion of when and why to use multiple partitions.
...
See the [authentication guide](./3.how-to-configure-authentication.md) for the format of the `authentication_datafile`.
...
configurations = {
  core = {
    env = {
      ...
      Amqp__MaxRetries                           = "5"
      Amqp__QueueStorage__LockRefreshPeriodicity = "00:00:45"  # How often to renew the task lease
      Amqp__QueueStorage__LockRefreshExtension   = "00:02:00"  # By how much each renewal extends the lease
      Amqp__QueueStorage__PollPeriodicity        = "00:00:10"  # How often the polling agent polls the queue
      ...
      # Submitter
      Submitter__DeletePayload = true  # Delete input payload from object storage after the task completes
    }
  }
  control = {
    env = {
...
- **Long-running tasks**: increase `Amqp__QueueStorage__LockRefreshExtension` so the lease does not expire mid-execution.
- **High throughput**: reduce `Amqp__QueueStorage__PollPeriodicity` so workers pick up tasks faster.
- **Cost optimisation**: reduce `MongoDB__DataRetention` and `Redis__TtlTimeSpan` to lower storage requirements; set `Submitter__DeletePayload = true` to free object storage immediately after task completion.
...
See [Personalizing the Admin GUI](./5.configure-gui.md) for the `gui_configuration` format.
