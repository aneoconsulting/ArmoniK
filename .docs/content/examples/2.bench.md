...
The available `BenchOptions` (env var = `BenchOptions__<Name>`) are summarized in the following table. For the up-to-date list, see the [`BenchOptions` class](https://github.com/aneoconsulting/ArmoniK.Core/blob/main/Tests/Bench/Client/src/Options/BenchOptions.cs) in ArmoniK.Core.
